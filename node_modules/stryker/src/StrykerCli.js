...
var StrykerCli = /** @class */ (function () {
    function StrykerCli(argv) {
        this.argv = argv;
        this.log = log4js_1.getLogger(StrykerCli.name);
        this.command = '';
    ...
    StrykerCli.prototype.run = function () {
        ...
        program
            ...
            .parse(this.argv);
        log4js_1.setGlobalLogLevel(program['logLevel'] || 'info');
        ...
        if (program['logLevel']) {
            log4js_1.setGlobalLogLevel(program['logLevel']);
        }
        var commands = {
        ...
        if (Object.keys(commands).indexOf(this.command) >= 0) {
            commands[this.command]().catch(function (err) {
                _this.log.error("an error occurred", err);
                process.exitCode = 1;
